package com.ningxiang.shop.auth.feign;
...
import com.ningxiang.shop.auth.mapper.AuthAccountMapper;
...
import com.ningxiang.shop.common.security.bo.AuthAccountInVerifyBO;
import com.ningxiang.shop.common.security.constant.InputUserNameEnum;
...
public class AccountFeignController implements AccountFeignClient {
    ...
    public ServerResponseEntity<Long> save(AuthAccountDTO authAccountDTO) {
        ServerResponseEntity<Long> segmentIdResponse = segmentFeignClient.getSegmentId("ningxiang-auth-account");
    ...
    public ServerResponseEntity<Void> updateUserInfoByUserIdAndSysType(UserInfoInTokenBO userInfoInTokenBO, Long userId, Integer sysType) {
        ...
        if (res != 1) {
            throw new NingxiangException("用户信息错误，更新失败");
        }
        return ServerResponseEntity.success();
    }
