...
我们用商品的服务`ningxiang-product`来举例，我们可以发现在`pom.xml`中依赖了`ningxiang-common-security`模块。
...
其中`tokenFeignClient` 是 `ningxiang-api-auth` 模块的方法，该接口其实是`feign`的一个接口，而实现就是`ningxiang-auth`进行实现。因为我们说过，我们的认证授权应该是一个统一的服务来的，而这个服务就是`ningxiang-auth`服务。也就是说项目启动，几乎是必须启动该项目先的。
...
### 用户角色权限
在用户角色权限的模型中，一个用户的权限往往是需要登录才能知道的。也细化到每个url，每个方法某个用户是否能够访问。我们的系统有的需要rbac模型，有的不需要，所以我们提取了一个rbac模型的服务`ningxiang-rbac`。我们回到`AuthFilter`，里面有一段
...
这里面的`permissionFeignClient` 其实也是一个feign服务，用于连接 `ningxiang-rbac` 这个服务，进行rbac模型的校验。
