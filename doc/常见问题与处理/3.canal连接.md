...
```sql
商品的同步sql为：
update ningxiang_product set update_time = now();


订单的数据同步sql为：
update ningxiang_order set update_time = now();
```
...
```properties
# 填写数据库地址
canal.instance.master.address=192.168.152.128:3306
...
```properties
# 填写RocketMQ地址
rocketmq.namesrv.addr = 192.168.152.128:9876
```
...
```shell
docker restart ningxiang-canal
```
...
es商品数据保存流程：ningxiang-product.spu表中的数据发生变动，canal监听到变动，发送mq，`ningxiang-search`服务消费mq， 并更新es中的商品数据


排查步骤：es中的商品数据没有更新，可以先检查`ningxiang-search`服务是否正常运行，如果正常运行，再看日志中mq的消费情况
