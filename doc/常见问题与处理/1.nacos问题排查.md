...
则通过 `docker logs -f ningxiang-nacos` 查看nacos的运行日志，根据错误信息将配置更改正确即可。
...
比如`nacos`上配置的redis地址是：`192.168.152.128`, 但服务启动日志中redis连接超时，且地址为`127.0.01`

步骤：
- 端口没有开放（8848、9848）， 浏览器中通过`ip:8848/nacos`可以访问nacos页面代表8848已经开放
- `nacos`配置列表中没有服务配置信息，重新导入`ningxiang-nacos.sql`, 导入`ningxiang-nacos`成功后，重启`nacos`容器
