...
onShow(() => {
  ...
  if (Data.isWechat) { // H5-微信环境
    if (!tempuid && !token) {
      ...
      login('/ningxiang_auth/ua/social/mp', code) // 先请求微信公众号登录接口
    } else {
      queryDataGroup()
    }
  } else {
...
const getIndexImgs = () => {
  uni.showLoading()
  http.request({
    url: '/ningxiang_multishop/ua/index_img/list',
    method: 'GET',
    data: {
...
const getSearchList = () => {
  ...
  http.request({
    url: '/ningxiang_search/ua/search/simple_page',
    method: 'GET',
...
const getPlatformCategoryList = () => {
  uni.showLoading()
  http.request({
    url: '/ningxiang_product/ua/category/category_list',
    method: 'GET',
    data: {
      shopId: 0,
      parentId: 0
