...
const handleEditOper = (operType) => {
  ...
  uni.showModal({
    ...
    success: function (res) {
      if (res.confirm) {
        http.request({
          url: '/ningxiang_product/a/shop_cart/delete_item',
          method: 'DELETE',
          data: selectIds
        }).then(() => {
          uni.showToast({
...
const changeNumber = (shopIndex, prodIndex, num) => {
  ...
  const params = {
    url: '/ningxiang_product/a/shop_cart/change_item',
    method: 'POST',
    data: reqData
  }
...
const getCartInfo = () => {
  http.request({
    url: '/ningxiang_product/a/shop_cart/info',
    method: 'GET',
    data: {}
  }).then((res) => {
...
const calTotalPrice = () => {
  const params = {
    url: '/ningxiang_product/a/shop_cart/check_items',
    method: 'POST',
...
const getProdInfo = (spuId) => {
  uni.showLoading()
  const params = {
    url: '/ningxiang_product/ua/spu/prod_info',
    method: 'GET',
    data: {
      spuId
    }
...
const getDefaultAddrStr = () => {
  uni.showLoading()
  const params = {
    url: '/ningxiang_user/user_addr/list',
    method: 'GET'
  }
  http.request(params).then(res => {
    if (!res.length) {
