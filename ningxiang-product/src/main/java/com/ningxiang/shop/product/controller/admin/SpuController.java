package com.ningxiang.shop.product.controller.admin;
...
import com.ningxiang.shop.product.dto.SkuDTO;
import com.ningxiang.shop.product.dto.SpuDTO;
import com.ningxiang.shop.product.dto.SpuPageSearchDTO;
import com.ningxiang.shop.product.model.SpuExtension;
import com.ningxiang.shop.product.service.*;
import com.ningxiang.shop.api.product.vo.*;
...
public class SpuController {
    ...
    private void spuUpdateStatus(SpuDTO spu) {
        ...
        if (StrUtil.isNotBlank(error)) {
            throw new NingxiangException(error);
        }
    ...
    private void spuBatchUpdateStatus(SpuDTO spu) {
        ...
        if (CollUtil.isEmpty(spuList)) {
            throw new NingxiangException("您选择的商品信息有误，请刷新后重试");
        }
        ...
        if (CollUtil.isEmpty(spuIds)) {
            throw new NingxiangException("您所选择的商品中没有符合操作条件的商品");
        }
        spuService.batchRemoveSpuCacheBySpuId(spuIds);
        if (errorList.size() > 0) {
            throw new NingxiangException("商品id为：" + errorList.toString() + "的" + errorList.size() + "件商品不符合操作条件");
        }
        spuService.changeSpuStatus(spu.getSpuId(), spu.getStatus());
        spuService.removeSpuCacheBySpuId(spu.getSpuId());
    }
    ...
    private void checkSaveOrUpdateInfo(SpuDTO spuDTO) {
        if (!Objects.equals(Constant.PLATFORM_SHOP_ID, AuthUserContext.get().getTenantId()) && Objects.isNull(spuDTO.getShopCategoryId())) {
            throw new NingxiangException("店铺分类不能为空");
        }
        if (Objects.isNull(spuDTO.getCategoryId())) {
            throw new NingxiangException("平台分类不能为空");
        }
