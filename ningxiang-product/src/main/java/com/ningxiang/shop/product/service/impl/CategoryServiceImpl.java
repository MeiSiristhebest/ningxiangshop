...
import com.ningxiang.shop.api.product.constant.CategoryLevel;
import com.ningxiang.shop.product.dto.CategoryDTO;
import com.ningxiang.shop.product.model.Category;
import com.ningxiang.shop.product.mapper.CategoryMapper;
...
//            throw new ningxiangException("分类不存在，请刷新后重试");
            return category;
        }
    ...
    public void update(Category category) {
        ...
        if (Objects.equals(dbCategory.getCategoryId(), category.getParentId())) {
            throw new NingxiangException("分类不能成为本身的上级分类");
        }
        existCategoryName(category);
    ...
    public void deleteById(Long categoryId) {
        ...
        if (count > 0) {
            throw new NingxiangException("该分类在使用中，不能进行删除操作");
        }
    ...
    private void existCategoryName(Category category) {
        ...
        if (countByName > 0) {
            throw new NingxiangException("分类名已存在，请重新输入");
        }
    }
}
